...
(function () {
  ...
  const CHANGELOG = [
    ...
    { v: '4.189.71', d: '2026-09-27', items: [
      ...
      '   · ถ้าไม่มีร้านใหม่ จะเดินต่อเนื่อง ไม่ต้องรอครบ cycle ที่ทุก waypoint เหมือนรุ่นก่อน',
      '⚡ ใช้ Vendor Actor Direct จาก v4.189.70 ต่อเนื่อง — เปิดเฉพาะ actor ที่มี Vendor Marker และยังใช้ single-pass ไม่มี retry',
      '   · ลด move polling เป็น ~160ms และ arrival radius เป็น 6 ช่อง เพื่อให้เส้นทางไหลลื่นขึ้น',
      '📊 Diagnostic เพิ่ม walkMs / scanMs / moveScanBatches เพื่อวัดเวลาที่เสียกับการเดินเทียบกับการเปิดร้านจริง',
      '   · ยังคง 3 ปุ่มตลาดเดิม: กวาดร้านใหม่ / อัปเดต >10 นาที / รีเฟรชทั้งหมด และเลือกเริ่มจากปลายเส้นทางที่ใกล้ตัวละครที่สุด',
    ]},
    { v: '4.189.70', d: '2026-09-27', items: [
      '🛠️ แก้ Vendor Flag Direct ID — Diagnostic v4.189.69 ยืนยันว่า OUT 0x6b ต้องส่ง Actor/Entity ID @7 ไม่ใช่ Shop Response ID 4 ไบต์ท้าย',
      '   · ใช้ marker 00 08 00 00 00 03 00 01 00 เป็นตัวกรองว่า entity นี้เปิดร้าน แล้วอ่าน Actor ID จาก offset 7 เพื่อเปิดร้าน',
      '   · 4 ไบต์ท้ายยังเก็บเป็น response/shop hint สำหรับเทียบ packet เท่านั้น ไม่เอาไปส่ง OUT 0x6b อีก',
      '⚡ Vendor-only candidate — ตัด player/entity ปกติออกก่อน probe และคง single-pass ไม่มี retry',
      '   · ปรับ timeout ร้านเป็น ~320ms เพื่อครอบคลุม response จริงที่เคยเห็น ~257ms โดยยังเร็วกว่า retry แบบเดิมมาก',
      '🧭 Sweep เริ่มจากปลายเส้นทางที่ใกล้ตัวละครที่สุด — ถ้าอยู่ฝั่งขวาตลาดจะย้อน 24 anchors แทนการเดินข้ามไปจุดแรกฝั่งซ้าย',
      '📡 Diagnostic เปลี่ยน label เป็น vendor_actor@7 + responseHint@end เพื่อดูจำนวน candidate ที่ถูกต้องในรอบถัดไป',
    ]},
    ...
    { v: '4.189.67', d: '2026-09-26', items: [
      ...
      '   · เพิ่มปุ่ม 📋 คัดลอกผล เพื่อส่งรายงาน Diagnostic มาให้วิเคราะห์และจูน Turbo Anchors/timeout รอบต่อไป',
    ]},
    { v: '4.189.66', d: '2026-09-26', items: [
      '🔎 Market Search Item Only — ช่องค้นหาค้นเฉพาะชื่อ Item และ Item ID',
      '   · เอาการค้นจากชื่อร้าน (Shop Name) ออก',
      '   · เอาการค้นจากชื่อผู้ขาย (Seller Name) ออก',
      '   · เปลี่ยน placeholder เป็น “ค้นชื่อ Item / Item ID” ให้ตรงกับพฤติกรรมจริง',
      '   · การเรียงราคาถูก → แพง / Pagination / Fast Market Scan / Smart Sweep ยังคงเดิม',
    ]},
    { v: '4.189.65', d: '2026-09-26', items: [
      '⚡ Smart Market Sweep — 43 จุดเดิม แต่ข้ามจุดที่ไม่มี Shop ID ใหม่โดยไม่รอเต็มเวลา',
      '   · เมื่อถึง waypoint จะเช็ก candidate ที่ยังไม่ถูกตรวจในรอบนี้ก่อน; ถ้าไม่มีจะรอสั้น ~120ms แล้วไปจุดถัดไปทันที',
      '   · ถ้ามี candidate ใหม่ ระบบจะรอแบบ quiet-window สั้น ๆ เพื่อรับ Shop ID ที่กำลังไหลเข้า แล้วค่อย Fast Scan',
      '   · Fast Scan 250ms + Retry 650ms / stale cache / New Shops Only จาก v4.189.61 ยังอยู่ครบ',
      '   · แสดงจำนวนจุดที่ข้ามเพราะไม่มี candidate ใหม่ในสถานะและ Log ตอนจบ',
      '   · เอาปุ่ม Stationary Scan Test ออกจากหน้า Market หลังผลทดสอบยืนยันแล้วว่าร้านโหลดตามตำแหน่ง',
    ]},
    { v: '4.189.64', d: '2026-09-26', items: [
      '🧪 Stationary Market Scan Test — เพิ่มโหมดทดสอบสแกนตลาดจากจุดเดียวโดยไม่เดิน',
      '   · ปิด automation ชั่วคราวและยืนตำแหน่งเดิม 5 วินาทีเพื่อดู Shop ID ที่ Client มีอยู่ ณ จุดนั้น',
      '   · แสดง candidate จาก packet buffer / Shop ID ที่เข้ามาระหว่างยืน / จำนวนร้านที่ยืนยันแล้วในฐาน',
      '   · Fast Scan เฉพาะ candidate ที่ยังไม่เคยยืนยัน ด้วย 250ms + Retry 650ms แบบ v4.189.61',
      '   · จำผลทดสอบครั้งก่อนของ prt_fild08 และรายงาน +ใหม่ / หาย เพื่อใช้เทียบเมื่อไปทดสอบอีกจุด',
      '   · ถ้าตัวละครเคลื่อนเกิน 2 ช่องระหว่างทดสอบจะขึ้น ⚠️ ผลไม่เป็น Stationary และไม่บันทึกเป็น baseline',
      '   · ปุ่มทดสอบกดซ้ำเพื่อ Stop ได้ และคืนสีเดิมทันทีตาม Button State Fix',
    ]},
    ...
    { v: '4.189.62', d: '2026-09-26', items: [
      ...
      '   · ♻️ อัปเดตราคา = เดิน Preset แล้วเปิดใหม่เฉพาะร้านที่ข้อมูลเก่าเกิน 10 นาที',
      '   · 🔄 รีเฟรชทั้งหมด = เดิน Preset แล้วบังคับอ่านร้านที่ค้นพบทั้งหมดใหม่',
      '   · เอา UI บันทึกจุด / ล้างจุด / กวาดตาม Saved Points / สแกนรอบตัวออกจากหน้า Market',
      '   · Market Index ไม่ถูกล้างเมื่อเปลี่ยนแมพอีกต่อไป เพื่อให้กลับมาตลาดแล้วใช้ฐานเดิมและสแกนเฉพาะส่วนที่ต้องอัปเดต',
      '   · ยังคง ASSIST.marketClearIndex() และ API รุ่นเก่าไว้เพื่อ compatibility แต่ workflow หน้า Market ใช้ 3 ปุ่มใหม่เป็นหลัก',
    ]},
    { v: '4.189.61', d: '2026-09-26', items: [
      '⚡ Fast Market Scan — เร่งการเก็บราคาด้วย 2-Pass แบบ sequential โดยไม่ยิงหลายร้านพร้อมกัน',
      '   · First pass รอร้าน ~250ms; ตัวที่ไม่ตอบจะเข้า Retry queue แล้วลองรอบสอง ~650ms',
      '   · จำ Shop ID ที่ไม่ตอบหลัง Retry เป็น stale ชั่วคราว 60s เพื่อลดการเสียเวลาเปิด candidate เดิมซ้ำ',
      '   · สแกน/กวาดปกติเน้น New Shops Only: ร้านที่มีในดัชนีแล้วจะถูกข้าม; เพิ่มปุ่ม ♻️ รีเฟรชทั้งหมด สำหรับบังคับตรวจร้านเดิมใหม่',
      '   · Market Sweep เปลี่ยนจากรอโหลดคงที่ 700ms เป็น Adaptive wait ~180-500ms ตามการเข้ามาของ Shop ID',
      '   · ลด delay หลังปิดร้าน/ระหว่าง candidate โดยยังคงส่ง request ทีละร้านเพื่อไม่ให้ response สลับกัน',
      '   · ล้าง Market Index จะล้าง stale cache ด้วย เพื่อให้สแกนร้านทั้งหมดใหม่ได้ทันที',
    ]},
    { v: '4.189.60', d: '2026-09-26', items: [
      '⚔️ Combat Master Flee — Combat เป็นสวิตช์หลักของระบบหนีฝั่งมอนทั้งหมด',
      '   · Combat OFF → ปิดการทำงานของ หนีมอนรุม / มอนอันตราย / Blacklist Flee / HP Emergency Flee ทันที',
      '   · สวิตช์ย่อยและค่าตั้งของแต่ละระบบยังจำไว้เหมือนเดิม; เปิด Combat กลับมาแล้วทำงานตามค่าเดิม',
      '   · Player Flee ยังคงแยกจาก Combat ตามเดิม',
      '   · HP Flee จะไม่ยิง Direct / Macro / Clip / Fly Wing และไม่ใช้ Unstuck ขณะ Combat OFF',
      '   · ปรับข้อความในหน้า Combat ให้ชัดว่าระบบหนีฝั่งมอนต้องมี Combat ON',
    ]},
    { v: '4.189.59', d: '2026-09-26', items: [
      '⏱️ Direct Cooldown Fallback — หลังใช้ Direct จะพัก Direct 30 วินาที แล้วข้ามไป Macro → Teleport Clip → Fly Wing ทันที',
      '   · ครบ 30 วินาทีแล้ว รอบวาร์ปถัดไปจะกลับมาเริ่มที่ Direct อัตโนมัติ',
      '   · ใช้กับ Warp Find, หนีมอนรุม, มอนอันตราย, HP Emergency Flee, Blacklist Flee และหนีผู้เล่น',
      '   · Direct ที่ถูกใช้จากระบบอื่นจะนับ cooldown เดียวกัน เพื่อไม่ลอง Direct ซ้ำในช่วงที่ server ยังไม่พร้อม',
      '   · คง serializer 3 วินาทีสำหรับ Direct ข้ามแมพ/พิกัดของ routine อื่นแยกจาก cooldown 30 วินาทีนี้',
    ]},
    { v: '4.189.58', d: '2026-09-26', items: [
      ...
      '   · หนีผู้เล่นแบบเปลี่ยนแมพ: Direct ไปแมพสำรองก่อน → ถ้า Direct ยัง cooldown 30s ใช้ Macro → Clip → Fly Wing ระหว่างรอ Direct พร้อม',
  ...
  // ★ v4.189.64 — Stationary Market Scan Test (ไม่เดิน)
  const MARKET_STATIONARY_TEST_KEY = 'ro_assist_market_stationary_test_v1';
  const MARKET_STATIONARY_OBSERVE_MS = 5000;
  let marketStationaryActive = false;
  let marketStationaryCancel = false;
  let marketStationaryResults = {}; // map -> {t,x,y,ids,liveIds,validated,checked,found}
  ...
  let marketDiagStart = null;
  let marketDiagPacketStats = new Map(); // key DIR:OP:LEN -> {count,sample}
  ...
  function marketBuildTurboAnchors(points, targetCount=MARKET_TURBO_ANCHOR_COUNT) {
    const pts=marketCleanRouteArray(points); if(pts.length<=2) return pts.slice();
    targetCount=Math.max(2,Math.min(pts.length,Math.round(Number(targetCount)||MARKET_TURBO_ANCHOR_COUNT)));
    if(pts.length<=targetCount) return pts.slice();
    const cum=[0];
    for(let i=1;i<pts.length;i++) cum.push(cum[i-1]+Math.hypot(pts[i].x-pts[i-1].x,pts[i].y-pts[i-1].y));
    const total=cum[cum.length-1]||1, out=[], used=new Set();
    for(let k=0;k<targetCount;k++){
      const target=total*k/(targetCount-1);
      let best=0,bestD=Infinity;
      for(let i=0;i<cum.length;i++){ const d=Math.abs(cum[i]-target); if(d<bestD){bestD=d;best=i;} }
      if(!used.has(best)){ used.add(best); out.push(pts[best]); }
    }
    const last=pts[pts.length-1];
    if(!out.length||out[out.length-1].x!==last.x||out[out.length-1].y!==last.y) out.push(last);
    return out;
  }

  function marketDiagHex(u, maxBytes=MARKET_DIAG_SAMPLE_BYTES) {
    if(!u||!u.length) return '';
    const n=Math.min(u.length,Math.max(8,Number(maxBytes)||MARKET_DIAG_SAMPLE_BYTES));
    let out=''; for(let i=0;i<n;i++) out+=(i?' ':'')+Number(u[i]).toString(16).padStart(2,'0');
    return out+(u.length>n?' …':'');
  }
  function marketDiagPacket(dir,u) {
    if(!marketDiagActive||!u||!u.length) return;
    const key=dir+':'+u[0]+':'+u.length;
  ...
  function marketDiagEvent(type,data={}) {
  ...
  function marketDiagFindCandidateSource(id) {
    ...
    // v4.189.70 direct Vendor Flag (Actor ID) ก่อน
    for(let i=marketProbePackets.length-1;i>=0;i--){
      ...
      if(marketReadVendorActorIdFromPacket(rec.data)===id) return rec;
    }
  ...
  function marketDiagBuildReport(reason='snapshot') {
    ...
    const end={map:marketNormalizeRouteMapName(currentMap),x:player.x==null?null:Math.round(Number(player.x)),y:player.y==null?null:Math.round(Number(player.y)),shops:marketShopIndex.size,listings:marketAllRows().length};
    const sig=marketShopIdSignature?('IN 0x'+Number(marketShopIdSignature.op).toString(16).padStart(2,'0')+' '+marketShopIdSignature.mode+'@'+marketShopIdSignature.offset+' support='+marketShopIdSignature.support):'none';
    const packets=[...marketDiagPacketStats.values()].sort((a,b)=>b.count-a.count);
    const marketPackets=packets.filter(r=>r.op===0x6b||r.op===0x6a||r.op===0x4d);
    const top=packets.slice(0,45);
    const lines=[
      'MARKET_DIAG v'+VERSION+' reason='+reason,
      'duration_ms='+dur,
    ...
    for(const e of marketDiagEvents){
      ...
      for(const [k,v] of Object.entries(e)) if(k!=='dt'&&k!=='type'&&v!=null) bits.push(k+'='+v);
  ...
  function marketDiagStartCapture() {
    ...
    marketScanStatus='📡 เริ่มจับ Market Packet แล้ว · กด ⚡ กวาดตลาด 1 รอบ จากนั้นกดหยุดจับ'; updateMarketUI();
    log('📡 Market Diagnostic: START'); return true;
  }
  function marketDiagStopCapture(reason='user') {
    if(!marketDiagActive) return marketDiagLastReport;
    marketDiagEvent('capture_stop',{reason});
    marketDiagLastReport=marketDiagBuildReport(reason); marketDiagActive=false;
    marketScanStatus='📡 จับข้อมูลแล้ว · กด 📋 คัดลอกผล แล้วส่งมาให้วิเคราะห์'; updateMarketUI();
    log('📡 Market Diagnostic: STOP · '+reason+' · report '+marketDiagLastReport.length+' chars');
    return marketDiagLastReport;
  }
  async function marketDiagCopyReport() {
    const txt=marketDiagActive?marketDiagBuildReport('live-snapshot'):marketDiagLastReport;
    if(!txt){ marketScanStatus='ยังไม่มีผล Diagnostic · กด 📡 จับ Packet ก่อน'; updateMarketUI(); return false; }
    let ok=false;
    try { if(navigator.clipboard&&navigator.clipboard.writeText){ await navigator.clipboard.writeText(txt); ok=true; } } catch(_) {}
    if(!ok){
      try { const ta=document.createElement('textarea'); ta.value=txt; ta.style.cssText='position:fixed;left:-9999px;top:-9999px'; document.body.appendChild(ta); ta.select(); ok=document.execCommand('copy'); ta.remove(); } catch(_) {}
    }
    marketScanStatus=ok?'📋 คัดลอก Diagnostic แล้ว · วางส่งมาในแชตได้เลย':'⚠️ Copy ไม่สำเร็จ · เปิด Console แล้วพิมพ์ ASSIST.marketDiagReport()'; updateMarketUI();
    return ok;
  }
  marketLoadRoutes();
  ...
  const MARKET_FAST_FIRST_TIMEOUT_MS = 320;
  ...
  function marketLoadStationaryTests() {
    try {
      const x=JSON.parse(localStorage.getItem(MARKET_STATIONARY_TEST_KEY)||'{}');
      if(x&&typeof x==='object') marketStationaryResults=x;
    } catch(_) { marketStationaryResults={}; }
  }
  function marketSaveStationaryTests() {
    try { localStorage.setItem(MARKET_STATIONARY_TEST_KEY, JSON.stringify(marketStationaryResults)); } catch(_) {}
  }
  ...
  // ★ v4.189.70 — Vendor Flag Direct Discovery (Actor ID fix)
  // Diagnostic v4.189.68/69 ยืนยันรูปแบบ:
  //   IN 0x06 ... actorEntityId@7 ... 00 08 00 00 00 03 00 01 00 <responseShopId:u32LE>
  // การเปิดร้าน OUT 0x6b ต้องส่ง actor/entity id @7; 4 ไบต์ท้ายเป็น response/shop hint เท่านั้น
  const MARKET_VENDOR_TRAILER = [0x00,0x08,0x00,0x00,0x00,0x03,0x00,0x01,0x00];
  let marketVendorResponseHintByActorId = new Map(); // actor entity id -> response/shop hint id

  function marketHasVendorTrailer(u) {
    if (!u || u.length < 13 || u[0] !== 0x06) return false;
    const m = u.length - 13;
    for (let i=0;i<MARKET_VENDOR_TRAILER.length;i++) if (u[m+i] !== MARKET_VENDOR_TRAILER[i]) return false;
    return true;
  }
  function marketReadVendorActorIdFromPacket(u) {
    if (!marketHasVendorTrailer(u) || u.length < 11) return 0;
    const id = u32(u, 7) >>> 0;
    return id || 0;
  }
  function marketReadVendorResponseHintFromPacket(u) {
    if (!marketHasVendorTrailer(u)) return 0;
  ...
  function marketDiscoverVendorOpenIds(limit=5000, maxAgeMs=MARKET_PROBE_TTL_MS) {
    ...
    for (const rec of marketProbePackets) {
      ...
      if (currentMap && rec.map && rec.map!==currentMap) continue;
      const id=marketReadVendorActorIdFromPacket(rec.data);
      if(!id||seen.has(id)||id===playerId) continue;
  ...
  marketLoadIndex();
  marketLoadDiscovery();
  marketLoadStationaryTests();

  function marketObserveOutgoing(u) {
    if (!u || !u.length) return;
    marketDiagPacket('OUT',u);
    if (u[0] === 0x6b && u.length === 5) {
      const entityId = u32(u, 1) >>> 0;
      const source = marketSweepActive ? 'sweep' : (marketScanActive ? 'scan' : (marketStationaryActive ? 'stationary' : (marketShopTravelActive ? 'select' : 'manual')));
  ...
  function marketClearIndex() {
    marketShopIndex.clear(); marketStaleOpenIds.clear(); marketSaveIndex(); updateMarketUI(); log('🧹 ล้าง Market Index + stale cache แล้ว');
  }

  // ★ v4.189.62 — ตลาดมีอยู่ที่ prt_fild08 แมพเดียว: เปลี่ยนแมพให้หยุดงาน แต่คงฐานราคาไว้
  function marketHandleMapChange(prevMap, nextMap, source) {
    if (!prevMap || !nextMap || prevMap === nextMap) return false;
    // หยุดงานที่กำลังวิ่งและล้างเฉพาะ state ชั่วคราวของแมพเก่า
    marketScanCancel = true;
    marketSweepCancel = true;
    marketStationaryCancel = true;
    marketShopTravelCancel = true;
    marketLastOpenRequest = null;
    marketScanWaiter = null;
    marketKnownOpenIds.clear();
    marketStaleOpenIds.clear();
    marketPage = 1;
    marketScanStatus = 'ฐานข้อมูลตลาดยังคงอยู่ · กลับ prt_fild08 แล้วกด กวาดตลาด / อัปเดตราคา ได้ทันที';
    const panel = document.getElementById('__assist_market_panel');
    if (panel) setTimeout(() => updateMarketUI(), 0); // currentMap จะถูกอัปเดตหลัง handler คืนค่า
    log('💾 Market: เปลี่ยนแมพ ' + prevMap + ' → ' + nextMap + ' (' + (source || 'map change') + ') · คงดัชนี ' + marketShopIndex.size + ' ร้านไว้');
    return true;
  }
  function marketFilteredRows(q) {
    ...
    if (q) rows=rows.filter(r=>{
      ...
      return id.includes(q) || nm.toLowerCase().includes(q);
    });
  ...
  async function marketSweepMoveTo(wp, startMap, idx, total, maxShops, policy='new') {
    const started=Date.now(); let lastProgress=Date.now(); let lx=player.x, ly=player.y;
    let scanMs=0, scanBatches=0, checked=0, found=0, skippedKnown=0, skippedFresh=0, skippedStale=0;
    marketDiagEvent('move_start',{i:idx+1,total,tx:wp.x,ty:wp.y,sx:player.x==null?'?':Math.round(player.x),sy:player.y==null?'?':Math.round(player.y)});
    while(!marketSweepCancel && marketSweepActive){
      if(currentMap!==startMap){
        ...
        marketDiagEvent('move_end',{i:idx+1,ok:0,reason:'map-change',moveMs:elapsed,walkMs,scanMs,scanBatches});
        return {ok:false,reason:'map-change',walkMs,scanMs,scanBatches,checked,found,skippedKnown,skippedFresh,skippedStale};
      }
      if(player.x==null||player.y==null){ await marketSleep(MARKET_TURBO_MOVE_POLL_MS); continue; }
      const dist=Math.hypot(wp.x-player.x,wp.y-player.y);

      // ★ v4.189.71 — Event-driven: Vendor ใหม่เข้ามาระหว่างเดิน → หยุดตรงจุดปัจจุบัน, scan, แล้วเดินต่อ
      const liveIds=marketSweepEligibleCandidateIds(policy);
      if(liveIds.length && marketShopIndex.size<maxShops){
        const batchStarted=Date.now();
        try { sendMove(Math.round(player.x),Math.round(player.y)); } catch(_) {}
        marketScanStatus='⚡ เจอ Vendor ใหม่ '+liveIds.length+' ระหว่างเดิน · หยุดสแกนที่ @('+Math.round(player.x)+','+Math.round(player.y)+')';
        renderMarketIndexUI();
        marketDiagEvent('move_vendor_seen',{i:idx+1,dist:dist.toFixed(1),candidates:liveIds.length,x:Math.round(player.x),y:Math.round(player.y)});
        await marketSleep(25);
        const sr=await marketSweepProbeIds(liveIds,maxShops,policy,'move');
        const spent=Date.now()-batchStarted; scanMs+=spent; scanBatches++;
      ...
      if(dist<=MARKET_TURBO_ARRIVE_RADIUS){
        const elapsed=Date.now()-started, walkMs=Math.max(0,elapsed-scanMs);
        marketDiagEvent('move_end',{i:idx+1,ok:1,moveMs:elapsed,walkMs,scanMs,scanBatches,dist:dist.toFixed(1),x:Math.round(player.x),y:Math.round(player.y)});
      ...
      marketScanStatus='🚀 เดินต่อเนื่อง '+(idx+1)+'/'+total+' → ('+wp.x+','+wp.y+') · เหลือ '+dist.toFixed(0)+' ช่อง · เจอ '+marketShopIndex.size+' ร้าน';
      renderMarketIndexUI();
      routineWalkTowardPoint(wp.x,wp.y,'🚀 Market Event Sweep');
      await marketSleep(MARKET_TURBO_MOVE_POLL_MS);
    }
    const elapsed=Date.now()-started, walkMs=Math.max(0,elapsed-scanMs);
    marketDiagEvent('move_end',{i:idx+1,ok:0,reason:'cancel',moveMs:elapsed,walkMs,scanMs,scanBatches});
    return {ok:false,reason:'cancel',walkMs,scanMs,scanBatches,checked,found,skippedKnown,skippedFresh,skippedStale};
  }
  ...
  function marketShopNeedsPriceUpdate(id, ageMs=MARKET_PRICE_STALE_MS) {
    const sh = marketFindIndexedVendorId(id, currentMap);
    return !!sh && (Date.now() - Number(sh.t || 0) >= Math.max(1000, Number(ageMs) || MARKET_PRICE_STALE_MS));
  }
  function marketCountPriceStale(mapName=currentMap, ageMs=MARKET_PRICE_STALE_MS) {
    const wantMap = marketNormalizeRouteMapName(mapName);
    let n=0; const now=Date.now();
    for (const sh of marketShopIndex.values()) {
      if (!sh) continue;
      if (wantMap && sh.map && marketNormalizeRouteMapName(sh.map) !== wantMap) continue;
      if (now - Number(sh.t || 0) >= Math.max(1000, Number(ageMs) || MARKET_PRICE_STALE_MS)) n++;
    }
    return n;
  }
  function marketPruneStaleIds(now=Date.now()) {
    for (const [id, failAt] of marketStaleOpenIds) {
      if (now - Number(failAt || 0) >= MARKET_FAST_STALE_TTL_MS) marketStaleOpenIds.delete(id);
    }
  }
  function marketIsStaleId(id, now=Date.now()) {
    marketPruneStaleIds(now);
    const t = marketStaleOpenIds.get(Number(id) >>> 0);
    return !!t && (now - t < MARKET_FAST_STALE_TTL_MS);
  }
  function marketMarkStaleId(id) {
    id = Number(id) >>> 0;
    if (id) marketStaleOpenIds.set(id, Date.now());
  }
  async function marketFastProbeShop(id, timeoutMs, source) {
    id = Number(id) >>> 0;
    if (!id || !activeWS || activeWS.readyState !== 1) return null;
    const probeStart=Date.now();
    marketLastOpenRequest = {entityId:id,t:probeStart,source:source||'scan'};
    const waitMs=Math.max(100, Number(timeoutMs)||MARKET_FAST_FIRST_TIMEOUT_MS);
    const waitShop = marketWaitShop(id, waitMs);
    if (!marketSendShopOpen(id)) { marketScanWaiter = null; return null; }
    const sh = await waitShop;
    const elapsed=Date.now()-probeStart;
    if(marketDiagActive && marketDiagProbeStats){
      if(sh){
    ...
    if (sh) {
      ...
      marketSendShopClose();
      await marketSleep(MARKET_FAST_CLOSE_DELAY_MS);
      return sh;
    }
    return null;
  }
  ...
  async function marketSmartWaypointLoadWait(policy='new') {
    const start=Date.now();
    const quickNoNewMs=55;    // Event-driven: ตอนถึง anchor รอเผื่อ packet ปลายทางเพียงสั้น ๆ
    const quietMs=70;         // ถ้ามี Vendor ใหม่ รอ packet เงียบเล็กน้อยก่อนเปิดร้าน
    const maxWait=220;
    let ids=marketSweepEligibleCandidateIds(policy);
    let lastCount=ids.length, lastChange=start;
    if(lastCount===0){
      while(Date.now()-start<quickNoNewMs){
        await marketSleep(30);
        ids=marketSweepEligibleCandidateIds(policy);
        if(ids.length>0){ lastCount=ids.length; lastChange=Date.now(); break; }
      }
      if(lastCount===0){ const r={waitMs:Date.now()-start,candidates:0,skip:true}; marketDiagEvent('shopid_wait',r); return r; }
    }
    while(Date.now()-start<maxWait){
      await marketSleep(35);
      ids=marketSweepEligibleCandidateIds(policy);
      if(ids.length!==lastCount){ lastCount=ids.length; lastChange=Date.now(); }
      if(Date.now()-lastChange>=quietMs) break;
    }
    const r={waitMs:Date.now()-start,candidates:lastCount,skip:false}; marketDiagEvent('shopid_wait',r); return r;
  }
  ...
  async function marketStationaryScanTest() {
    if(marketStationaryActive){
      marketStationaryCancel=true;
      marketScanStatus='กำลังหยุด Stationary Test…';
      updateMarketUI();
      return false;
    }
    if(marketSweepActive||marketScanActive||marketShopTravelActive){
      marketScanStatus='หยุดงาน Market อื่นก่อนเริ่ม Stationary Test'; updateMarketUI(); return false;
    }
    if(!activeWS||activeWS.readyState!==1){ marketScanStatus='Stationary Test: WebSocket เกมยังไม่พร้อม'; updateMarketUI(); return false; }
    if(!currentMap||player.x==null||player.y==null){ marketScanStatus='Stationary Test: ยังไม่รู้แมพ/พิกัด'; updateMarketUI(); return false; }
    if(!marketShopIdSignature){
      marketScanStatus='Stationary Test: ยังไม่ learned Shop-ID · เปิดร้านด้วยมือ 1 ร้านก่อน'; updateMarketUI(); return false;
    }
    const mapKey=marketNormalizeRouteMapName(currentMap);
    const sx=Math.round(Number(player.x)), sy=Math.round(Number(player.y));
    const started=Date.now();
    marketStationaryActive=true; marketStationaryCancel=false;
    marketScanPauseAutomation();
    updateMarketUI();
    try{
      let maxMoved=0;
      while(!marketStationaryCancel && Date.now()-started<MARKET_STATIONARY_OBSERVE_MS){
        if(marketNormalizeRouteMapName(currentMap)!==mapKey){ marketStationaryCancel=true; break; }
        if(player.x!=null&&player.y!=null) maxMoved=Math.max(maxMoved,Math.hypot(Number(player.x)-sx,Number(player.y)-sy));
        const elapsed=Date.now()-started;
        const bufferCount=marketDiscoverIdsBySignature(marketShopIdSignature,5000).length;
        const liveCount=marketDiscoverIdsSince(marketShopIdSignature,started,5000).length;
        marketScanStatus='🧪 ยืนสแกน '+Math.min(5,Math.ceil(elapsed/1000))+'/5วิ · candidate '+bufferCount+' · live +'+liveCount+' · ขยับ '+maxMoved.toFixed(1)+' ช่อง';
        renderMarketIndexUI(); updateMarketUI();
        await marketSleep(250);
      }
      if(marketStationaryCancel){ marketScanStatus='Stationary Test หยุดแล้ว'; return false; }

      const ids=marketDiscoverIdsBySignature(marketShopIdSignature,5000).map(x=>Number(x)>>>0).filter(Boolean);
      const liveIds=marketDiscoverIdsSince(marketShopIdSignature,started,5000).map(x=>Number(x)>>>0).filter(Boolean);
      const prev=marketStationaryResults[mapKey]||null;
      const prevSet=new Set(Array.isArray(prev&&prev.ids)?prev.ids.map(x=>Number(x)>>>0):[]);
      ...
      if(player.x!=null&&player.y!=null) maxMoved=Math.max(maxMoved,Math.hypot(Number(player.x)-sx,Number(player.y)-sy));
      const moved=maxMoved>2;
      if(!moved){
        marketStationaryResults[mapKey]={t:Date.now(),x:sx,y:sy,ids:[...curSet],liveIds:[...new Set(liveIds)],validated,checked,found};
        marketSaveStationaryTests();
      }
      const delta=prev ? (' · เทียบครั้งก่อน +'+added+' / -'+removed) : ' · baseline ครั้งแรก';
      marketScanStatus=(moved?'⚠️ TEST INVALID: มีการเคลื่อนที่ ':'✅ Stationary Test: ')
        +'candidate '+ids.length+' · live +'+liveIds.length+' · ร้านยืนยัน '+validated+' · ตรวจใหม่ '+checked+' · เจอใหม่ '+found+delta
        +(moved?' · ขยับ '+maxMoved.toFixed(1)+' ช่อง':'');
      log((moved?'⚠️':'✅')+' Stationary Market Test @('+sx+','+sy+') — candidate '+ids.length+' / live '+liveIds.length+' / validated '+validated+' / checked '+checked+' / found '+found+(prev?' / delta +'+added+' -'+removed:' / baseline'));
      return {map:mapKey,x:sx,y:sy,candidates:ids.length,live:liveIds.length,validated,checked,found,retried,added,removed,moved,ids};
    } finally {
      marketStationaryActive=false; marketStationaryCancel=false; marketScanWaiter=null;
      marketScanRestoreAutomation(); renderMarketIndexUI(); updateMarketUI();
    }
  }

  async function marketSweepProbeIds(ids, maxShops, policy='new', source='sweep') {
    maxShops = marketNormalizeShopLimit(maxShops);
    policy = ['new','stale','all'].includes(policy) ? policy : 'new';
    marketPruneStaleIds();
    const started=Date.now();
    let checked=0,found=0,retried=0,skippedKnown=0,skippedFresh=0,skippedStale=0;
    for(const id0 of (Array.isArray(ids)?ids:[])){
      ...
      checked++;
      const sh=await marketFastProbeShop(id,MARKET_FAST_FIRST_TIMEOUT_MS,(source||'sweep')+'-'+policy);
      if(sh) found++; else marketMarkStaleId(id);
      await marketSleep(MARKET_FAST_BETWEEN_MS);
    }
    return {checked,found,retried,skippedKnown,skippedFresh,skippedStale,scanMs:Date.now()-started};
  }
  ...
  async function marketSweepScanRecent(maxShops, policy='new') {
    ...
    policy = ['new','stale','all'].includes(policy) ? policy : 'new';
    marketPruneStaleIds();
    const ids=marketSweepEligibleCandidateIds(policy);
    const result=await marketSweepProbeIds(ids,maxShops,policy,'anchor');
    marketDiagEvent('shop_scan',{source:'anchor',checked:result.checked,found:result.found,retried:result.retried,skippedKnown:result.skippedKnown,skippedFresh:result.skippedFresh,skippedStale:result.skippedStale,scanMs:result.scanMs});
    return result;
  }
  ...
  async function marketSweepMap(maxShops, policy='new') {
    ...
    maxShops=marketNormalizeShopLimit(maxShops);
    const startMap=currentMap;
    const preset = marketPresetForMap(startMap);
    if(!preset){ marketScanStatus='3 ปุ่มตลาดใช้ Preset กวาดตลาดล่างพรอน และทำงานเฉพาะ prt_fild08'; renderMarketIndexUI(); return false; }
    if(policy==='stale'){
      const staleCount=marketCountPriceStale(startMap, MARKET_PRICE_STALE_MS);
      if(!staleCount){
        marketScanStatus='✅ ยังไม่มีร้านที่ข้อมูลเก่าเกิน 10 นาที · ไม่ต้องอัปเดต'; renderMarketIndexUI();
        log('✅ Market Update: ไม่มีร้านเก่าเกิน 10 นาที'); return true;
      }
    }
    ...
    try{
      ...
      // ★ v4.189.70 — เริ่มจากปลายเส้นทางที่ใกล้ตัวละครที่สุด เพื่อลดการเดินข้ามตลาดก่อนเริ่ม
      if (marketSweepWaypoints.length > 1 && player.x != null && player.y != null) {
        const a=marketSweepWaypoints[0], b=marketSweepWaypoints[marketSweepWaypoints.length-1];
        const dA=Math.hypot(Number(player.x)-Number(a.x),Number(player.y)-Number(a.y));
        const dB=Math.hypot(Number(player.x)-Number(b.x),Number(player.y)-Number(b.y));
        if(dB<dA){
          marketSweepWaypoints.reverse();
          marketDiagEvent('route_reverse',{reason:'nearest-end',dFirst:dA.toFixed(1),dLast:dB.toFixed(1)});
        }
      }
      ...
      for(let i=0;i<marketSweepWaypoints.length;i++){
        ...
        const mv=await marketSweepMoveTo(wp,startMap,i,marketSweepWaypoints.length,maxShops,policy);
        totalWalkMs+=Number(mv.walkMs||0); totalScanMs+=Number(mv.scanMs||0); totalMoveScanBatches+=Number(mv.scanBatches||0);
        totalChecked+=Number(mv.checked||0); totalFound+=Number(mv.found||0); totalSkipKnown+=Number(mv.skippedKnown||0); totalSkipFresh+=Number(mv.skippedFresh||0); totalSkipStale+=Number(mv.skippedStale||0);
        if(!mv.ok){
          if(mv.reason==='map-change'||mv.reason==='cancel') break;
          skipped++; log('⏭️ Market: ข้ามจุด '+(i+1)+'/'+marketSweepWaypoints.length+' @('+wp.x+','+wp.y+') — '+mv.reason); continue;
        }
        marketScanStatus=policyLabel+' · จุด '+(i+1)+'/'+marketSweepWaypoints.length+' · เช็ก Shop ID ใหม่…'; renderMarketIndexUI();
        const smart=await marketSmartWaypointLoadWait(policy);
        if(smart.skip){
          smartSkipped++;
          marketScanStatus=policyLabel+' · จุด '+(i+1)+'/'+marketSweepWaypoints.length+' · ไม่มี candidate ใหม่ → ไปต่อ · ข้ามเร็ว '+smartSkipped+' จุด';
          renderMarketIndexUI();
          continue;
        }
        const sr=await marketSweepScanRecent(maxShops,policy);
        totalChecked+=sr.checked; totalFound+=sr.found; totalRetry+=sr.retried; totalSkipKnown+=sr.skippedKnown; totalSkipFresh+=sr.skippedFresh; totalSkipStale+=sr.skippedStale; totalScanMs+=Number(sr.scanMs||0);
        marketScanStatus=policyLabel+' · anchor '+(i+1)+'/'+marketSweepWaypoints.length+' · ตรวจ '+totalChecked+' · อัปเดต '+totalFound+' · move-scan '+totalMoveScanBatches+' ครั้ง'; renderMarketIndexUI();
        await marketSleep(30);
      }
  ...
  async function marketScanVisible(maxShops, refreshAll=false) {
    if(marketShopTravelActive){ marketScanStatus='กำลังเดินไปเปิดร้านอยู่'; renderMarketIndexUI(); return false; }
    ...
    for(const id0 of ids){
      const id=Number(id0)>>>0;
      if(!id) continue;
      if(!refreshAll && marketHasIndexedVendorId(id)){ skippedKnown++; continue; }
      if(!refreshAll && marketIsStaleId(id)){ skippedStale++; continue; }
      candidates.push(id);
    }
  ...
  function updateMarketUI() {
    ...
    if (!panel) return;
    renderMarketIndexUI();
    const newBtn = panel.querySelector('[data-market-work-new]');
    const staleBtn = panel.querySelector('[data-market-work-stale]');
    const allBtn = panel.querySelector('[data-market-work-all]');
    const diagBtn = panel.querySelector('[data-market-diag-toggle]');
    const diagCopyBtn = panel.querySelector('[data-market-diag-copy]');
    const routeTxt = panel.querySelector('[data-market-route-status]');
    const preset=marketPresetForMap();
    const staleCount=marketCountPriceStale(currentMap, MARKET_PRICE_STALE_MS);
    if(routeTxt) routeTxt.textContent = preset
      ? ('Preset: ✅ '+preset.name+' · Event '+Math.min(MARKET_TURBO_ANCHOR_COUNT,preset.points.length)+'/'+preset.points.length+' จุดนำทาง · ฐาน '+marketShopIndex.size+' ร้าน · เก่า>10น '+staleCount+' ร้าน')
    ...
    setupBtn(allBtn,'all','🔄 รีเฟรชทั้งหมด');
    if(diagBtn){
      diagBtn.textContent=marketDiagActive?'⏹ หยุดจับ Packet':'📡 จับ Packet ตลาด';
      diagBtn.style.background=marketDiagActive?'#5a2026':'#38264d';
      diagBtn.style.color=marketDiagActive?'#ffcdd2':'#ce93d8';
      diagBtn.style.borderColor=marketDiagActive?'#b94a54':'#70438a';
    }
    if(diagCopyBtn){ diagCopyBtn.disabled=!marketDiagLastReport&&!marketDiagActive; diagCopyBtn.style.opacity=diagCopyBtn.disabled?'.4':'1'; diagCopyBtn.style.cursor=diagCopyBtn.disabled?'not-allowed':'pointer'; }
  }

  function openMarketPanel() {
    // ★ v4.189.62: Market Index คงอยู่ข้ามการเปลี่ยนแมพ; 3 ปุ่มใช้ Preset prt_fild08 อัตโนมัติ
    ...
    panel.innerHTML=`
      ...
      <div style="background:#181824;border-radius:8px;padding:7px;min-height:0;display:flex;flex-direction:column;flex:1 1 auto;overflow:hidden">
        <input data-market-search placeholder="ค้นชื่อ Item / Item ID" style="width:100%;box-sizing:border-box;background:#0d0d15;color:#eee;border:1px solid #3a3f4b;border-radius:6px;padding:6px 7px;font-size:10px;margin-bottom:5px;flex:0 0 auto">
        <div style="display:grid;grid-template-columns:1fr;gap:4px;margin-bottom:4px;flex:0 0 auto">
          <button data-market-work-new title="Turbo Sweep ตลาดล่างพรอน 24 anchors และเปิดเฉพาะร้านใหม่ที่ยังไม่มีในฐาน" style="width:100%;background:#124a3a;color:#80cbc4;border:1px solid #287a66;border-radius:6px;padding:6px 7px;cursor:pointer;font-size:9px">⚡ กวาดตลาด</button>
          <button data-market-work-stale title="Turbo Sweep 24 anchors และอัปเดตเฉพาะร้านที่ข้อมูลเก่าเกิน 10 นาที" style="width:100%;background:#3b2f14;color:#ffd54f;border:1px solid #7d6424;border-radius:6px;padding:6px 7px;cursor:pointer;font-size:9px">♻️ อัปเดตราคา >10นาที</button>
          <button data-market-work-all title="Turbo Sweep 24 anchors และบังคับเปิดร้านที่ค้นพบทั้งหมดใหม่" style="width:100%;background:#263a58;color:#90caf9;border:1px solid #41688f;border-radius:6px;padding:6px 7px;cursor:pointer;font-size:9px">🔄 รีเฟรชทั้งหมด</button>
        </div>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:4px;margin-bottom:4px;flex:0 0 auto">
          <button data-market-diag-toggle title="จับ packet + timing ระหว่างกวาด เพื่อส่งผลมาจูนความเร็ว" style="background:#38264d;color:#ce93d8;border:1px solid #70438a;border-radius:6px;padding:5px 6px;cursor:pointer;font-size:8px">📡 จับ Packet ตลาด</button>
          <button data-market-diag-copy title="คัดลอกรายงาน Diagnostic ล่าสุดไป Clipboard" style="background:#252535;color:#b0bec5;border:1px solid #45455a;border-radius:6px;padding:5px 6px;cursor:pointer;font-size:8px">📋 คัดลอกผล</button>
        </div>
        <div data-market-route-status style="font-size:8px;color:#a5d6a7;margin-bottom:4px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;flex:0 0 auto">Preset: กวาดตลาดล่างพรอน · Turbo 24/43 จุด</div>
    ...
    panel.querySelector('[data-market-work-new]').onclick=()=> marketSweepMap('all', 'new');
    panel.querySelector('[data-market-work-stale]').onclick=()=> marketSweepMap('all', 'stale');
    panel.querySelector('[data-market-work-all]').onclick=()=> marketSweepMap('all', 'all');
    panel.querySelector('[data-market-diag-toggle]').onclick=()=>{ if(marketDiagActive) marketDiagStopCapture('user'); else marketDiagStartCapture(); updateMarketUI(); };
    panel.querySelector('[data-market-diag-copy]').onclick=()=> marketDiagCopyReport();
    updateMarketUI();
    renderMarketIndexUI();
    if (!marketScanActive && !marketSweepActive) log('🔎 Market: เปิดหน้าต่าง · ฐานข้อมูลคงอยู่ข้ามแมพ · ใช้ 3 ปุ่มกับ Preset ตลาดล่างพรอน · Turbo 24/43 anchors · มี Packet Diagnostic สำหรับจูนความเร็ว');
  }
  ...
  const teleportMacroWatcher = setInterval(() => {
    ...
    if (!p) return;
    // ★ v4.189.60 — ถ้า Combat ถูกปิดกลางคัน ให้ยกเลิก Macro ของระบบ Combat/Flee (Player Flee/Test ยังเป็นอิสระ)
    if (!CFG.combatEnabled && ['warpfind','hp','blacklist','monster-flee'].includes(p.owner)) {
      teleportMacroPending = null;
      dbg('🛑 ยกเลิก Teleport Macro ของ ' + p.owner + ' เพราะ Combat OFF');
      return;
    }
    const now = nowMs();
  ...
  // ★ v4.189.59 — Direct/Database TP มี reuse cooldown 30s สำหรับลำดับวาร์ปอัตโนมัติ
  // ระหว่าง cooldown ให้ข้าม Direct แล้วไป Macro → Clip → Fly Wing ทันที
  // หมายเหตุ: serializer 3s ด้านบนยังคงใช้กับ routine teleport ข้ามแมพ/พิกัดทั่วไป
  const DIRECT_PRIORITY_COOLDOWN_MS = 30000;
  function directPriorityCooldownLeft(now) {
    const t = now == null ? nowMs() : now;
    return Math.max(0, DIRECT_PRIORITY_COOLDOWN_MS - (t - lastTeleportSentAt));
  }
  function directPriorityReady(now) { return directPriorityCooldownLeft(now) <= 0; }
  ...
  function hpFleeSameMap() {
    ...
    const dbReady = !!currentMap && directPriorityReady(now);
    if (dbReady) {
      ...
      const left = directPriorityCooldownLeft(now);
      dbg('❤️ HP Flee: Direct TP ยัง cooldown ' + Math.ceil(left / 1000) + 's → Macro');
    }
    // Priority 2 → 4: Macro → Clip → Wing
    return hpFleeTryMacroThenClipWing(dbReady ? 'Direct ส่งไม่สำเร็จ' : 'Direct ยัง cooldown 30s');
  }
  function triggerHpEmergencyFlee(pct) {
    if (!CFG.combatEnabled) return false; // ★ v4.189.60 Combat master
  ...
  const hpEmergencyFleeLoop = setInterval(() => {
    if (chatPauseActive) return;
    // ★ v4.189.60 — Combat เป็น Master Switch ของ HP Emergency Flee
    if (!CFG.combatEnabled) {
      hpFleeLatched = false;
      hpFleePendingClip = null;
      hpFleeNextTryAt = 0;
  ...
  // ★★ v4.189.59 — Player Flee fallback เมื่อ Direct ยังอยู่ใน cooldown 30s
  ...
  function playerFleeStartSameMap(nearby) {
    const now = nowMs();
    const directReady = !!currentMap && directPriorityReady(now);
    if (directReady) {
      ...
      const left = directPriorityCooldownLeft(now);
      dbg('🏃 หนีผู้เล่นแมพเดิม: Direct ยัง cooldown ' + Math.ceil(left / 1000) + 's → Macro');
    }
    return playerFleeSameMapTryMacro(nearby, directReady ? 'Direct ส่งไม่สำเร็จ' : 'Direct ยัง cooldown 30s');
  }
  function playerFleeStartChangeMap(nextMap, nearby) {
    ...
    if (directReady) {
      ...
      const left = directPriorityCooldownLeft(now);
      log('⚡ หนีผู้เล่น: Direct เปลี่ยนแมพยัง cooldown ~' + Math.ceil(left / 1000) + 's → Macro → Clip → Wing แล้วค่อยเปลี่ยนแมพ');
    }
    // อย่าเรียก sendTeleport(nextMap) ระหว่าง Direct cooldown 30s; ให้ fallback หนีก่อน
    // เราต้องหนีทันทีตามลำดับ Macro → Clip → Wing ก่อน แล้วค่อยยิง Direct เองเมื่อ cooldown ครบ
    pendingTeleport = null;
    return playerFleeTryMacro(nextMap, nearby, directReady ? 'Direct ส่งไม่สำเร็จ' : 'Direct ยัง cooldown 30s');
  }
  const playerFleeFallbackLoop = setInterval(() => {
    ...
    if (p.phase === 'mapwait') {
      ...
      if (!directPriorityReady(now)) return;
      pendingTeleport = null;
      CFG.farmMap = p.map;
      saveConfigDebounced();
  ...
  function blacklistFleeSameMap(label) {
    ...
    if (dbReady) {
      ...
      const left = directPriorityCooldownLeft(now);
      dbg('🌀 Blacklist Flee: Direct TP ยัง cooldown ' + Math.ceil(left / 1000) + 's → Macro');
    }
    return blacklistFleeTryMacroThenClipWing(label, dbReady ? 'Direct ส่งไม่สำเร็จ' : 'Direct ยัง cooldown 30s');
  }
  function checkBlacklistAttackFlee() {
    if (!CFG.combatEnabled) { blacklistFleePendingClip = null; return false; } // ★ Combat master
  ...
  function doFlee(reason) {
    ...
    if (directReady) {
      ...
      const left = directPriorityCooldownLeft(now);
      dbg('🏃 หนีมอน: Direct ยัง cooldown ' + Math.ceil(left / 1000) + 's → Macro');
    }
    return monsterFleeTryMacroThenClipWing(reason, directReady ? 'Direct ส่งไม่สำเร็จ' : 'Direct ยัง cooldown 30s');
  }

  const monsterFleeFallbackLoop = setInterval(() => {
    if (!CFG.combatEnabled) { monsterFleePendingClip = null; return; }
  ...
  // ★ v4.189.60 — Mob Flee ถูกควบคุมด้วย Combat master guard ก่อนเรียกฟังก์ชันนี้
  ...
  function checkMobFleeTriggers() {
    if (!CFG.combatEnabled) return false; // ★ Combat master
  ...
  // ★ v4.189.60 — Dangerous Monster Flee อยู่ใต้ Combat master เช่นเดียวกับ Mob Flee
  ...
  // Direct map warp ยัง cooldown 30s → Macro → Clip → Wing เพื่อหนีทันที → ครบ cooldown ค่อยเปลี่ยนแมพสำรอง
  ...
  const combatLoop = setInterval(() => {
      ...
      if (now >= fleeCooldownUntil) {
        ...
        if (player.x != null && currentMap) {
          ...
          if (nearby > 0) {
            ...
            // ★★ changeMap mode — Direct → Macro → Clip → Wing fallback เมื่อ Direct ยัง cooldown 30s
    ...
    // ★ v4.189.60 — Combat เป็น Master Switch ของระบบหนีฝั่งมอนทั้งหมด
    // Player Flee ด้านบนยังแยกจาก Combat ตามเดิม แต่ HP/Mob/Danger/Blacklist Flee จะไม่ทำงานเมื่อ Combat OFF
    if (!CFG.combatEnabled) return;

    // sync ตำแหน่งสำหรับ trigger ที่ใช้รัศมี เมื่อ Combat ON เท่านั้น
    if (player.x == null && playerId != null) {
      const me = entities.get(playerId);
      if (me && me.x != null) { player.x = me.x; player.y = me.y; }
    }
    if (player.x == null && playerId != null && activeWS && activeWS.readyState === 1) {
      if (now - (lastWhereReqAt || 0) > 5000) {
        if (sendWhere()) {
          lastWhereReqAt = now;
          dbg('📍 ตำแหน่งยังไม่รู้ → ถาม /where สำหรับ Combat Flee');
        }
      }
    }
    ...
    // ★ Combat Flee (มอนอันตราย + มอนรุม) ถูกตรวจหลัง Combat master guard (v4.189.60)
  ...
  window.ASSIST = {
    ...
    combatOff() {
      ...
      blacklistFleePendingClip = null; blacklistFleeNextTryAt = 0;
      monsterFleePendingClip = null;
      if (teleportMacroPending && ['warpfind','hp','blacklist','monster-flee'].includes(teleportMacroPending.owner)) teleportMacroClear();
      clearUnstuckBuffAutoFinishPending();
      ...
      if (typeof unstuckBuffLastAt !== 'undefined' && typeof unstuckBuffState !== 'undefined' && unstuckBuffState === 'IDLE' && !unstuckBuffManualRun) unstuckBuffLastAt = 0;
      log('⚔️ Auto-Combat: OFF · ปิด Combat Flee ทั้งหมด (HP/Blacklist/Danger/Mob) + บล็อก Warp Find; Player Flee ยังแยกตามเดิม');
    },
    ...
    setHpFleeMode(mode) { CFG.hpFleeMode = mode === 'unstuck' ? 'unstuck' : 'sameMap'; hpFleeLatched = false; hpFleePendingClip = null; saveConfigDebounced(); log('❤️ HP Flee mode =', CFG.hpFleeMode === 'unstuck' ? 'Unstuck 0x73' : 'หนีในแมพ (Direct → Macro → Clip → Wing)'); },
    ...
    marketScan(maxShops) { openMarketPanel(); return marketScanVisible(maxShops == null ? 'all' : maxShops, false); },
    marketScanRefresh(maxShops) { openMarketPanel(); return marketScanVisible(maxShops == null ? 'all' : maxShops, true); },
    marketSweepNew() { openMarketPanel(); return marketSweepMap('all', 'new'); },
    marketUpdatePrices() { openMarketPanel(); return marketSweepMap('all', 'stale'); },
    marketRefreshAll() { openMarketPanel(); return marketSweepMap('all', 'all'); },
    marketSweep(maxShops) { openMarketPanel(); return marketSweepMap(maxShops == null ? 'all' : maxShops, 'new'); },
    marketSweepPronLower(maxShops) { openMarketPanel(); return marketSweepMap(maxShops == null ? 'all' : maxShops, 'new'); },
    ...
    marketDiagReport() { return marketDiagActive ? marketDiagBuildReport('live-snapshot') : marketDiagLastReport; },
    marketDiagCopy() { return marketDiagCopyReport(); },
  ...
  function buildUI() {
      ...
      <div id="__assist_popup">
        ...
        <div class="__assist_page" data-page="config">
          ...
          <div class="__assist_subpage" data-sub="combat">
            ...
            <div style="font-size:10px;color:#9aa0a6;margin-top:4px;line-height:1.5">★ ปุ่ม OFF = ปิด trigger รุม/aggro/มอนรอบทั้งหมดชั่วคราว แต่ไม่ลบค่าที่ตั้งไว้<br>★ ปุ่ม ON = กลับมาใช้ threshold เดิมทันที · ทำงานเมื่อ ⚔️ Combat ON<br>★ ลำดับหนี: Direct → Macro → Teleport Clip → Fly Wing · วิธีที่ใช้ไม่ได้จะถูกข้าม</div>
            ...
            <div style="font-size:10px;color:#9aa0a6;margin-top:4px;line-height:1.5">★ OFF = หยุดตรวจรายชื่อมอนอันตรายชั่วคราว แต่ยังจำรายชื่อและระยะไว้<br>★ ทำงานเมื่อ ⚔️ Combat ON เช่นเดียวกับหนีมอนรุม<br>★ ลำดับหนี: Direct → Macro → Teleport Clip → Fly Wing · วิธีที่ใช้ไม่ได้จะถูกข้าม</div>
            <hr style="border:none;border-top:1px solid #3a3f4b;margin:10px 0">
            ...
            <div style="font-size:10px;color:#9aa0a6;margin-top:4px;line-height:1.5">★ หนีในแมพ: Direct/Database TP (0x40) → Fixed Hotkey Macro → Teleport Clip → Fly Wing 601 · วิธีที่ใช้ไม่ได้จะถูกข้าม<br>★ Unstuck: ส่ง 0x73 ทันที 1 ครั้ง · ทำงานเมื่อ ⚔️ Combat ON</div>
          ...
          <div class="__assist_subpage" data-sub="flee">
            ...
            <div style="font-size:10px;color:#9aa0a6;margin-top:4px;line-height:1.5">★ ทุกโหมดหนีผู้เล่นใช้ลำดับ Direct → Macro → Teleport Clip → Fly Wing · โหมดเปลี่ยนแมพจะยิง Direct ไปแมพสำรองก่อน; ถ้า Direct ยัง cooldown 30s จะใช้ fallback ระหว่างรอ</div>
          </div>
          <!-- 🪑 Rest -->
    ...
    // ---- Combat Flee wires (v4.189.60: Combat เป็น Master Switch; สวิตช์ย่อยยังจำค่าเดิม) ----
